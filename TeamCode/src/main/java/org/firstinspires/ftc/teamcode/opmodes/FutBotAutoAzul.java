...
/**
 * OpMode autónomo de FutBot con objetivo de anotar en la PORTERÍA AZUL.
 *
 * Máquina de estados priorizada (mayor número = menor prioridad):
 *   P0 - Supervivencia : evadir la línea blanca
 *   P1 - Ataque        : perseguir la pelota con control proporcional
 *   P2 - Anotación     : empujar la pelota hacia la portería azul
...
public class FutBotAutoAzul extends LinearOpMode {
    ...
    private static final int ID_PORTERIA_OBJETIVO = FutBotVision.ID_PORTERIA_AZUL;
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if (hw.detectaBlancoNorte()) {
                // Frente detecta línea → retroceder
                telemetry.addData("Evasion", "NORTE - Retrocediendo");
                telemetry.update();
                hw.setPoderMotores(-PODER_AVANCE, -PODER_AVANCE);
                sleep(TIEMPO_EVASION_MS);
                hw.detener();
                tiempoRadar.reset(); // reiniciar radar tras evasión
                continue; // reinicia el ciclo; no ejecuta prioridades inferiores

            } else if (hw.detectaBlancoSur()) {
                // Trasero detecta línea → avanzar
            ...
            } else if (hw.detectaBlancoOeste()) {
                // Lado izquierdo detecta línea → girar a la derecha
                telemetry.addData("Evasion", "OESTE - Girando derecha");
                telemetry.update();
            ...
            } else if (hw.detectaBlancoEste()) {
                // Lado derecho detecta línea → girar a la izquierda
                telemetry.addData("Evasion", "ESTE - Girando izquierda");
                telemetry.update();
                hw.setPoderMotores(-PODER_GIRO_RADAR, PODER_GIRO_RADAR);
                sleep(TIEMPO_EVASION_MS);
                hw.detener();
            ...
            // Se evalúa antes de P1 porque si ya tenemos la pelota controlada,
            // la tarea es dirigirse a la portería, no seguir persiguiendo la pelota.
            // =================================================================
                ...
                if (vision.hayPorteria(ID_PORTERIA_OBJETIVO)) {
                    ...
                    telemetry.addData("Estado", "ANOTANDO hacia porteria azul");
                    telemetry.addData("Error porteria (px)", errorPorteria);

                } else {
                    // Pelota controlada pero portería no visible: girar buscándola.
                    hw.setPoderMotores(PODER_GIRO_RADAR, -PODER_GIRO_RADAR);
                    telemetry.addData("Estado", "BUSCANDO porteria azul (pelota controlada)");
                }
            ...
            // =================================================================
                ...
                // Control diferencial: un motor acelera, el otro frena proporcionalmente.
                double poderIzq = PODER_AVANCE + correccion;
                double poderDer = PODER_AVANCE - correccion;

                hw.setPoderMotores(poderIzq, poderDer);

                // Una vez que la pelota es visible, el radar se mantiene en reset.
                tiempoRadar.reset();
            ...
            // =================================================================
                ...
                if (vision.hayPorteria(ID_PORTERIA_OBJETIVO)) {
                    // Portería visible: avanzar hacia ella para cambiar ángulo de visión.
                    telemetry.addData("Estado", "REPOSICIONANDO hacia porteria azul");
                    telemetry.update();
                    ...
                    // Sin portería: girar buscando cualquier referencia visual.
                    hw.setPoderMotores(PODER_GIRO_RADAR, -PODER_GIRO_RADAR);
                    telemetry.addData("Estado", "REPOSICIONANDO - Buscando porteria");
                }

                // Reiniciar el radar para dar otra oportunidad de búsqueda normal.
                tiempoRadar.reset();
